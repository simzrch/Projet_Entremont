...
class AccueilPage(Page):
    ...
    def domaine_qualiter(self):
        if self.auth_system.logged_in_user is not None:
            qualite_page.show()
            self.hide()
        else:
            QMessageBox.warning(self.ui, "Non connecté", "Veuillez vous connecter d'abord.")
    ...
    def hide_parametre(self):
        if self.auth_system.logged_in_user.role == 2:
            self.ui.ButtonParametres.show()
        elif self.auth_system.logged_in_user.role == 3:
            self.ui.ButtonParametres.show()
...
class QualitePage(Page):
    def __init__(self, ui_file, auth_system):
        ...
        self.ui.ButtonRestriction.clicked.connect(self.show_role_when_clicked)

        self.setup_ui_connections()

    
    def show_role_when_clicked(self):    
        if self.auth_system.logged_in_user is not None:
            QMessageBox.information(self.ui, "Role de l'utilisateur", f"Role: {self.auth_system.logged_in_user.role}")
        else:
            QMessageBox.warning(self.ui, "Login Failed", "User not logged in")
...
class HygienePage(Page):
    def __init__(self, ui_file, auth_system):
        ...
        self.ui.ButtonRestriction.clicked.connect(self.show_role_when_clickede)

        self.setup_ui_connections()

    
    def show_role_when_clickede(self):    
        if self.auth_system.logged_in_user is not None:
            QMessageBox.information(self.ui, "Role de l'utilisateur", f"Role: {self.auth_system.logged_in_user.role}")
        else:
            QMessageBox.warning(self.ui, "Login Failed", "User not logged in")
...
class LoginWindow(QWidget):
    ...
    def check_login(self):
        ...
        if self.auth_system.verifier_authentification(username, password):
            ...
            self.accueil_page.ui.ButtonQualiter.setEnabled(True)
            self.accueil_page.ui.ButtonHygiene.setEnabled(True)
